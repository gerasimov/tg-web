import { MessageCreator } from 'app/core/mtproto/MessageCreator';
import {
  strDecToHex
} from 'app/core/mtproto/utils';
import { bytesFromHex, bytesToHex } from 'app/core/mtproto/crypto/shared'
import { sha1BytesSync } from 'app/core/mtproto/crypto/sha1'

const publicKeysHex = [
  {
    modulus:
      '',
    exponent: '010001'
  },
  // },
  // {
  //   modulus:
  //     '',
  //   exponent: '010001',
  // },
  // {
  //   modulus:
  //     '',
  //   exponent: '010001',
  // },
  // {
  //   modulus:
  //     '',
  //   exponent: '010001',
  // },
  // {
  //   modulus:
  //     '',
  //   exponent: '010001',
  // },
];

const publicKeysParsed: any = {};
let prepared = false;

prepareRsaKeys();

export function prepareRsaKeys() {
  if (prepared) {
    return;
  }

  for (let i = 0; i < publicKeysHex.length; i++) {
    const keyParsed = publicKeysHex[i];

    const RSABuffer = new MessageCreator(320)
      .bytes(bytesFromHex(keyParsed.modulus))
      .bytes(bytesFromHex(keyParsed.exponent)).pack();

    const fingerprintBytes = sha1BytesSync(RSABuffer).slice(-8);
    fingerprintBytes.reverse();
    
    publicKeysParsed[bytesToHex(fingerprintBytes)] = keyParsed;
  }

  prepared = true;
}

export function selectRsaKeyByFingerPrint(fingerprints: string[]) {
  for (let i = 0; i < fingerprints.length; i++) {
    let fingerprintHex = strDecToHex(fingerprints[i]);
    let foundKey = publicKeysParsed[fingerprintHex];
    if (foundKey) {
      return Object.assign({}, { fingerprint: fingerprints[i] }, foundKey);
    }
  }

  return false;
}
